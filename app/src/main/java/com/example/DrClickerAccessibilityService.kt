...
class DrClickerAccessibilityService : AccessibilityService() {
     ...
     */
    private fun triggerHumanlikeGesture(target: NodeInfoData, isUrgent: Boolean = false, forceSwipe: Boolean = false) {
        ...
        DrClickerController.logEvent(
            "🛡️ [ANTI-BAN ACTIVE] Scheduling gesture: Mode=$mode, Delay=${delayMs}ms, Coords=($targetX, $targetY), PressDur=${touchDuration}ms, ForceSwipe=$forceSwipe",
            true
        )
        ...
        handler.postDelayed({
            // Check points left
            if (!DrClickerController.deductPoint()) {
                DrClickerController.logEvent("⚠️ Gesture cancelled: Points balance became 0 during scheduling.", false)
                return@postDelayed
            }
            ...
            val isSlideToAccept = forceSwipe ||
                                  target.text.uppercase().contains("SLIDE") || 
                                  target.text.uppercase().contains("SWIPE") ||
                                  target.contentDescription.uppercase().contains("SLIDE") ||
                                  target.contentDescription.uppercase().contains("SWIPE")
